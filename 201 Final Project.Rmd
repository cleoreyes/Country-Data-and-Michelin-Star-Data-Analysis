...
```{r}
countrydatamichelin %>% 
  ggplot(aes(x = `Minimum wage`, y = `Sum Stars`, size = `Sum Stars`, col = `Sum Stars`)) +
...
```{r}
countrydatamichelin %>% ggplot(aes(x = GDP, y = `Sum Stars`, size = `Sum Stars`, col = `Sum Stars`)) + 
  geom_point(alpha = 0.3, na.rm = TRUE) + 
  scale_size_continuous(range = c(1, 20)) +
...
```{r}
countrydatamichelin %>%
  ggplot(aes(x = Population, y = `Sum Stars`, size = `Sum Stars`, col = `Sum Stars`)) +
  geom_point(alpha = 0.3, na.rm = TRUE) + 
  scale_size_continuous(range = c(1, 20)) +
  labs(title = "Population vs Michelin Star Count", 
       x = "Population", 
       y = "Total Michelin Stars", 
       size = "Number of Stars",
       color = "Number of Stars")
...
geom_text_repel(aes(label = Country),
                  box.padding   = 0.35, 
                  point.padding = 0.5,
                  segment.color = 'grey50') + labs(title = "Number of Stars vs Price Rating", x = "Price Rating", y = "Number of Stars")
``````
